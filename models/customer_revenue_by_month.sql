...
final as (
    ...
    select
        ...
        mrr > 0 as is_active,
        --case when mrr > 0 then 1 end as is_active,
    ...
    max(case when mrr > 0 then date_month end) over (
        ...
        min(case when mrr > 0 then date_month end) over (partition by customer_id) = date_month as is_first_month,
        max(case when mrr > 0 then date_month end) over (partition by customer_id) = date_month as is_last_month
